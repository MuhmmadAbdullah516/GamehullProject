  ...
  return (
    <AuthLayout>
      <section className="w-full max-w-[440px] rounded-[24px] border border-blue-500/25 bg-[linear-gradient(180deg,rgba(3,18,31,0.94)_0%,rgba(15,9,69,0.96)_58%,rgba(31,11,80,0.98)_100%)] p-[clamp(28px,5vw,44px)] shadow-[0_24px_80px_rgba(0,0,0,0.42)]">
          ...
          <div className="grid grid-cols-4 gap-2">
            {otp.map((digit, index) => (
              <input
                ...
                className="h-12 min-w-0 rounded-xl border border-blue-300/15 bg-white/8 text-center text-sm font-bold leading-[21px] text-white outline-none transition placeholder:text-slate-400 focus:border-blue-500/70 focus:ring-2 focus:ring-blue-500/25"
                inputMode="numeric"
                key={index}
                maxLength={1}
                name={`otp-${index + 1}`}
        ...
        <div className="mt-7 flex items-center justify-center gap-2 text-[13px] font-normal leading-5 text-white/55">
