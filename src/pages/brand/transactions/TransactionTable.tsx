          ...
          <tbody className="divide-y divide-slate-900/10 dark:divide-blue-400/15">
            {transactions.map((tx) => (
              <tr className="transition-colors hover:bg-slate-100/40 dark:hover:bg-white/[0.04]" key={tx.id}>
                <td className="px-6 py-4"><div className="flex items-center gap-3"><TransactionIcon type={tx.type} /><div><div className="text-[13.5px] font-bold text-slate-900 dark:text-white">{tx.type}</div><div className="font-mono text-[11.5px] text-slate-400 dark:text-zinc-600">{tx.id}</div></div></div></td>
                <td className="px-6 py-4 text-[13.5px] text-slate-600 dark:text-zinc-400">{tx.details}</td>
                <td className="px-6 py-4"><div className="text-[13px] text-slate-500 dark:text-zinc-400">{tx.date}</div><div className="mt-0.5 text-[11.5px] text-slate-400 dark:text-zinc-600">{tx.time}</div></td>
                <td className="px-6 py-4 text-sm font-black text-slate-900 dark:text-white">{tx.amount}</td>
                <td className="px-6 py-4"><span className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide ${statusClass(tx.status)}`}>{tx.status}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      <div className="flex items-center justify-between border-t border-slate-900/10 bg-slate-100/40 px-6 py-4 text-[13px] text-slate-500 dark:border-blue-400/15 dark:bg-white/[0.03] dark:text-zinc-400">
        <span>Showing {start} to {end} of {totalCount} transactions</span>
        {totalPages > 1 ? (
          <div className="flex items-center gap-1.5">
            {Array.from({ length: totalPages }, (_, index) => index + 1).map((page) => (
              <button className={`flex size-8 cursor-pointer items-center justify-center rounded-lg text-[13px] font-bold transition ${currentPage === page ? "bg-blue-600 text-white shadow-md" : "border border-slate-900/10 text-slate-500 hover:bg-slate-100 dark:border-blue-400/15 dark:text-zinc-400 dark:hover:bg-white/[0.06]"}`} key={page} onClick={() => onPageChange(page)} type="button">{page}</button>
            ))}
          </div>
        ) : null}
      </div>
