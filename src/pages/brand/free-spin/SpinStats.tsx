    ...
    <section className="mt-12 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
      {stats.map((stat) => (
        <article
          ...
          key={stat.label}
        >
          <strong className="mb-1 block text-2xl font-black text-[#2563EB] dark:!text-[#3B82F6]">
            {stat.value}
          </strong>
          <span className="text-sm font-bold uppercase tracking-wider text-[#64748B] dark:!text-white/55">
            {stat.label}
          </span>
        </article>
      ))}
