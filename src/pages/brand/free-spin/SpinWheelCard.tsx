...
function SpinWheelCard(props: SpinWheelCardProps) {
  ...
  return (
    <section className="grid min-h-96 grid-cols-[minmax(0,1fr)_415px] items-center gap-14 rounded-4xl border border-slate-900/10 bg-white bg-[linear-gradient(135deg,rgba(37,99,235,0.04)_0%,transparent_100%)] px-9 py-12 shadow-[0_2px_16px_rgb(15_23_42_/_0.07)] transition duration-300 hover:border-blue-600/20 dark:border-blue-400/20 dark:bg-[#0e1629] dark:bg-[linear-gradient(135deg,rgba(37,99,235,0.08)_0%,transparent_100%)] max-lg:grid-cols-1 max-lg:gap-12 max-lg:p-8">
      <div>
        <span className="inline-flex h-7 items-center rounded-full border border-blue-600/20 bg-blue-600/[0.07] px-3.5 text-sm font-bold text-[#1D4ED8] dark:border-blue-400/25 dark:bg-blue-600/10 dark:!text-[#BFDBFE]">Daily Bonus</span>
        <h2 className="mb-4 mt-5 max-w-145 text-3xl md:text-4xl font-black leading-tight tracking-normal text-[#0F172A] dark:!text-[#FFFFFF]">Spin the Wheel & Win</h2>
        <p className="max-w-xl text-base leading-relaxed text-[#475569] dark:!text-[#A1A1AA]">
      ...
      <div className="grid place-items-center max-lg:-order-1">
          ...
          <div className="relative flex size-72 items-center justify-center rounded-full bg-[linear-gradient(135deg,#2563eb,#7c3aed)] p-1 opacity-80 shadow-xl">
            <div className="relative h-full w-full overflow-hidden rounded-full bg-white transition-transform duration-[4000ms] dark:bg-[#0f172a]" style={wheelStyle}>
              {prizes.map((prize, index) => (
                <span className="pointer-events-none absolute left-1/2 top-1/2 select-none text-2xl font-black text-[#64748B] dark:!text-white/55" key={`${prize.label}-${index}`} style={{ transform: `translate(-50%,-50%) rotate(${index * 45}deg) translateY(-100px)` }}>
                  {prize.label}
                </span>
              ))}
            </div>
        ...
        {props.totalSpinsAvailable <= 0 ? (
          <p className="mt-4 max-w-65 text-center text-sm leading-relaxed text-[#64748B] dark:!text-white/55">
