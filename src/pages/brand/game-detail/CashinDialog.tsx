...
function CashinDialog({ gameName, onComplete }: CashinDialogProps) {
  ...
  return (
    <DialogContent className="wallet-scrollbar max-h-[calc(100vh-40px)] max-w-[27.5rem] overflow-y-auto rounded-3xl border border-white/10 bg-[#0f172a] p-6 text-center text-white shadow-2xl dark:bg-[#020617] md:p-8">
      {step === "amount" ? (
        <button
          className="absolute left-6 top-6 flex cursor-pointer items-center gap-2 text-sm font-semibold text-white/60 transition-colors hover:text-white"
      ...
      <DialogClose asChild>
        <button
          aria-label="Close cashin popup"
          className="absolute right-6 top-6 flex size-10 cursor-pointer items-center justify-center rounded-full bg-white/5 text-white/60 transition-colors hover:bg-white/10 hover:text-white"
          type="button"
        >
      ...
      <div className="flex flex-col items-center text-center">
        <div className="mb-6 flex size-16 items-center justify-center rounded-full border border-primary/20 bg-primary/10 text-primary">
      ...
      {step === "source" ? (
        <div className="mt-8">
          ...
          <div className="space-y-3">
            <button
              className="group flex w-full cursor-pointer items-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-5 text-left transition-all hover:border-white/20 hover:bg-white/10"
            ...
            <button
              className="flex w-full cursor-not-allowed items-center gap-4 rounded-2xl border border-white/5 bg-white/[0.02] p-5 text-left opacity-50"
              disabled
              type="button"
            >
        ...
        <div className="mt-8 space-y-4 text-left">
          ...
          <div>
            <label
              className="mb-2 ml-1 block text-sm font-bold text-white/60"
            ...
            <div className="relative">
              ...
              <input
                className="h-14 w-full appearance-none rounded-xl border border-white/10 bg-transparent pl-10 pr-4 text-xl font-bold text-white outline-none transition-colors placeholder:text-white/40 focus:border-blue-500/70 focus:ring-2 focus:ring-blue-500/25"
                id="cashin-amount"
                inputMode="numeric"
                min={1}
          ...
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-5">
            {cashinPresetAmounts.map((presetAmount) => (
              <button
                className={cn(
                  "h-10 cursor-pointer rounded-lg border border-white/10 bg-white/5 text-sm font-bold text-white/60 transition-all hover:bg-white/10 hover:text-white",
          ...
          <Button
            className="mt-4 !h-14 w-full gap-2 rounded-xl bg-primary text-base font-bold text-white shadow-lg shadow-primary/20 transition-all hover:-translate-y-0.5 hover:bg-primary-hover disabled:cursor-wait disabled:opacity-70 disabled:hover:translate-y-0"
          ...
          <p className="mx-auto mt-6 max-w-[17.5rem] text-center text-sm leading-relaxed text-white/40">
            Funds will be deducted from your wallet and added to your {gameName}{" "}
            account.
          </p>
        </div>
      )}
